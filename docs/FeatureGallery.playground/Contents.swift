...
let galleryAssetVersion = "animation-gif"
let previewImageWidth = 144
let videoPreviewSlugs: Set<String> = [
	"animate",
	"animate-transform",
	"set-animation",
]
...
let gradientAndPatternExamples = [
	example("gradient-linear", "<linearGradient>", "Gradients and Patterns", .rendered, "Linear gradient paint servers render natively.") {
		##"<defs><linearGradient id="g"><stop offset="0" stop-color="#38bdf8"/><stop offset="1" stop-color="#ec4899"/></linearGradient></defs><rect width="120" height="80" fill="url(#g)"/>"##
	},
	example("gradient-radial", "<radialGradient>", "Gradients and Patterns", .rendered, "Radial gradient paint servers render natively.") {
		##"<defs><radialGradient id="g"><stop offset="0" stop-color="#fef08a"/><stop offset="1" stop-color="#f97316"/></radialGradient></defs><rect width="120" height="80" fill="url(#g)"/>"##
	},
	example("gradient-stop-offset", "<stop> offset", "Gradients and Patterns", .rendered, "Stop offsets control native gradient interpolation.") {
		##"<defs><linearGradient id="g"><stop offset="0.2" stop-color="#22c55e"/><stop offset="0.8" stop-color="#2563eb"/></linearGradient></defs><rect width="120" height="80" fill="url(#g)"/>"##
	},
	example("gradient-stop-opacity", "stop-opacity", "Gradients and Patterns", .rendered, "Stop opacity contributes to native gradient stop alpha.") {
		##"<defs><linearGradient id="g"><stop offset="0" stop-color="#ef4444" stop-opacity="0.1"/><stop offset="1" stop-color="#ef4444" stop-opacity="1"/></linearGradient></defs><rect width="120" height="80" fill="url(#g)"/>"##
	},
	example("gradient-object-bounding-box", "gradientUnits objectBoundingBox", "Gradients and Patterns", .rendered, "Object-bounding-box gradient units map through the target bounds.") {
		##"<defs><linearGradient id="g" gradientUnits="objectBoundingBox" x1="0" x2="1"><stop offset="0" stop-color="#38bdf8"/><stop offset="1" stop-color="#0f172a"/></linearGradient></defs><rect width="120" height="80" fill="url(#g)"/>"##
	},
	example("gradient-user-space", "gradientUnits userSpaceOnUse", "Gradients and Patterns", .rendered, "User-space gradient units render in SVG user coordinates.") {
		##"<defs><linearGradient id="g" gradientUnits="userSpaceOnUse" x1="0" x2="120"><stop offset="0" stop-color="#f97316"/><stop offset="1" stop-color="#7c2d12"/></linearGradient></defs><rect width="120" height="80" fill="url(#g)"/>"##
	},
	example("gradient-transform", "gradientTransform", "Gradients and Patterns", .rendered, "Gradient transforms are applied to native gradient geometry.") {
		##"<defs><linearGradient id="g" gradientTransform="rotate(20)"><stop offset="0" stop-color="#22c55e"/><stop offset="1" stop-color="#a855f7"/></linearGradient></defs><rect width="120" height="80" fill="url(#g)"/>"##
	},
	example("gradient-spread-pad", "spreadMethod pad", "Gradients and Patterns", .rendered, "Pad spread extends endpoint colors natively.") {
		##"<defs><linearGradient id="g" x2=".35" spreadMethod="pad"><stop offset="0" stop-color="#38bdf8"/><stop offset="1" stop-color="#2563eb"/></linearGradient></defs><rect width="120" height="80" fill="url(#g)"/>"##
	},
	example("gradient-spread-reflect", "spreadMethod reflect", "Gradients and Patterns", .rendered, "Reflect spread mirrors gradient stops natively.") {
		##"<defs><linearGradient id="g" x2=".35" spreadMethod="reflect"><stop offset="0" stop-color="#38bdf8"/><stop offset="1" stop-color="#2563eb"/></linearGradient></defs><rect width="120" height="80" fill="url(#g)"/>"##
	},
...
func example(
	...
	FeatureExample(
		...
		width: width,
		height: height,
		hasVideoPreview: videoPreviewSlugs.contains(slug),
...
enum FeatureGalleryRenderer {
	...
	static func renderAll(examples: [FeatureExample], packageRoot: URL, pngDirectory explicitPNGDirectory: URL? = nil) throws {
		...
		let gifDirectory = packageRoot.appendingPathComponent("docs/feature-gallery/gif", isDirectory: true)
		let mp4Directory = packageRoot.appendingPathComponent("docs/feature-gallery/mp4", isDirectory: true)
		...
		try FileManager.default.createDirectory(at: gifDirectory, withIntermediateDirectories: true)
		...
		for example in examples {
			...
			if example.rendererStatus == .rendered, example.hasVideoPreview {
				let gifURL = gifDirectory.appendingPathComponent("\(example.slug).gif")
				try renderGIF(document, gifURL: gifURL, width: example.width, height: example.height)
				print("Rendered \(gifURL.path)")
				let mp4URL = mp4Directory.appendingPathComponent("\(example.slug).mp4")
				try renderVideo(document, mp4URL: mp4URL, width: example.width, height: example.height)
	...
	private static func writeIndex(examples: [FeatureExample], packageRoot: URL) throws {
		...
		Generated by `docs/FeatureGallery.playground/Contents.swift`. Each SVG is parsed by `swg` and rendered through the public `SVG` SwiftUI view; dynamic animation examples also include GIF previews linked to MP4 loops.
		...
		for example in examples {
			...
			let preview = previewHTML(for: example, pathPrefix: "")
			markdown += """
			<tr>
	...
	static func previewHTML(for example: FeatureExample, pathPrefix: String) -> String {
		guard example.rendererStatus == .rendered else { return "" }
		let alt = htmlEscaped(example.title)
		let pngPath = "\(pathPrefix)png/\(example.slug).png?v=\(galleryAssetVersion)"
		if example.hasVideoPreview {
			let gifPath = "\(pathPrefix)gif/\(example.slug).gif?v=\(galleryAssetVersion)"
			let mp4Path = "\(pathPrefix)mp4/\(example.slug).mp4?v=\(galleryAssetVersion)"
			return "<a href=\"\(mp4Path)\"><img src=\"\(gifPath)\" alt=\"\(alt)\" width=\"\(previewImageWidth)\"></a>"
		}
		return "<img src=\"\(pngPath)\" alt=\"\(alt)\" width=\"\(previewImageWidth)\">"
	}
	...
	private static func render(_ document: SVGDocument, pngURL: URL, width: Int, height: Int) throws {
		let image = try renderImage(document, width: width, height: height, animationTime: document.animations.isEmpty ? nil : 1.5)
	...
	@MainActor
	private static func renderVideo(_ document: SVGDocument, mp4URL: URL, width: Int, height: Int) throws {
		let frameRate = 30
		let duration = 2
		...
		guard writer.canAdd(input) else {
			throw FeatureGalleryError.invalidRenderedVideo(mp4URL)
		}
		writer.add(input)
		writer.startWriting()
		...
		for frame in 0..<frameCount {
			...
			let animationTime = Double(frame) / Double(frameRate)
			let image = try renderImage(document, width: width, height: height, animationTime: animationTime)
	...
	@MainActor
	private static func renderGIF(_ document: SVGDocument, gifURL: URL, width: Int, height: Int) throws {
		let frameRate = 15
		let duration = 2
		let frameCount = frameRate * duration
		...
		guard let destination = CGImageDestinationCreateWithURL(gifURL as CFURL, UTType.gif.identifier as CFString, frameCount, nil) else {
			throw FeatureGalleryError.invalidRenderedGIF(gifURL)
		}
		...
		let frameProperties = [
			kCGImagePropertyGIFDictionary: [
				kCGImagePropertyGIFDelayTime: 1 / Double(frameRate),
			],
		] as CFDictionary
		for frame in 0..<frameCount {
			let animationTime = Double(frame) / Double(frameRate)
	...
	@MainActor
	private static func renderImage(_ document: SVGDocument, width: Int, height: Int, animationTime: TimeInterval?) throws -> CGImage {
		let options = SVGRenderOptions(animationTime: animationTime)
		let renderer = ImageRenderer(content: SVG(document, options: options).frame(width: CGFloat(width), height: CGFloat(height)))
		renderer.scale = 1
		guard let image = renderer.cgImage else {
			throw FeatureGalleryError.renderingFailed("image")
		}
		return image
	}
	...
	private static func makePixelBuffer(from image: CGImage, width: Int, height: Int) throws -> CVPixelBuffer {
		...
		guard status == kCVReturnSuccess, let pixelBuffer else {
			throw FeatureGalleryError.invalidRenderedVideo(URL(fileURLWithPath: "pixel-buffer"))
		}
		...
		guard let context = CGContext(
			...
			throw FeatureGalleryError.invalidRenderedVideo(URL(fileURLWithPath: "bitmap-context"))
		}
...
enum FeatureGalleryError: Error {
	...
	case invalidRenderedGIF(URL)
	case invalidRenderedVideo(URL)
}

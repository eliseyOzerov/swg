...
let galleryAssetVersion = "animation-video"
...
enum FeatureGalleryRenderer {
	...
	private static func writeIndex(examples: [FeatureExample], packageRoot: URL) throws {
		...
		Generated by `docs/FeatureGallery.playground/Contents.swift`. Each SVG is parsed by `swg` and rendered through the public `SVG` SwiftUI view; dynamic animation examples also include MP4 loops.
	...
	static func previewHTML(for example: FeatureExample, pathPrefix: String) -> String {
		...
		if example.hasVideoPreview {
			...
			return "<video src=\"\(mp4Path)\" poster=\"\(pngPath)\" width=\"\(previewImageWidth)\" autoplay loop muted playsinline controls aria-label=\"\(alt)\"></video>"
		}

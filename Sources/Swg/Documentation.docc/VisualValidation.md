...
The first visual fixtures cover basic paint, transforms, paint servers, clipping, masking, local markers, clock-based animation snapshots, and basic text through the ``SVG`` view.

Future visual fixtures should be added as rendering support expands through paint order, fill rules, strokes, advanced paint-server behavior, filters, advanced text layout, contextual marker paint, motion paths, image handling, reuse, and viewport behavior.
